...
class admin:
    ...
    def cart_info(self,code=None):
      self.sharing_price=0
      if code is not None:
        temp_code = self.db.referal_code_table.find({'code': code})
        if temp_code is not None:
          for i in temp_code:
            self.sharing_price=i['sharing_price']
            return self.sharing_price
...
def view_cart():
    ...
    return render_template("cart.html", cart=cart1,total_price=total_price)
